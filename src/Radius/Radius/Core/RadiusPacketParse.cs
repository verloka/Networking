...
namespace Radius.Core
{
    public class RadiusPacketParser : IRadiusPacketParser
    {
        ...
        /// RadiusPacketParser
        /// </summary>
        public RadiusPacketParser(IRadiusDictionary radiusDictionary)
        {
        ...
        public IRadiusPacket Parse(byte[] packetBytes, byte[] sharedSecret)
        {
            ...
            while (position < packetLength)
            {
                ...
                try
                {
                    if (typecode == 26) // VSA
                    {
                        ...
                        if (vendorAttributeDefinition == null)
                        {
                            ...
                            //$"Unknown vsa: {vsa.VendorId}:{vsa.VendorCode}"
                        }
                        else
                        {
                            try
                            ...
                            {
                                //TODO log output here
                                //$"Something went wrong with vsa {vendorAttributeDefinition.Name}"
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            //TODO log output here
                            //$"Something went wrong with {attributeDefinition.Name}"
                            //$"Attribute bytes: {contentBytes.ToHexString()}"
                        }
                    }
                }
                ...
                {
                    //TODO log output here
                    //$"Attribute {typecode} not found in dictionary"
                }
                catch (Exception ex)
                {
                    //TODO log output here
                    //$"Something went wrong parsing attribute {typecode}"
                }
        ...
        public bool TryParsePacketFromStream(Stream stream, out IRadiusPacket packet, byte[] sharedSecret)
        {
            ...
            if (i != 0)
            {
                ...
                {
                    //TODO log output here
                    //"Unable to parse packet from stream"
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        byte[] CalculateMessageAuthenticator(byte[] packetBytes, byte[] sharedSecret, byte[] requestAuthenticator)
        {
        ...
        byte[] CalculateResponseAuthenticator(byte[] sharedSecret, byte[] requestAuthenticator, byte[] packetBytes)
        {
        ...
        /// <returns></returns>
        byte[] GetAttributeValueBytes(object value)
        {
            switch (value)
            {
                case string _value:
